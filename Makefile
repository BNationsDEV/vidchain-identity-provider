...
hydra:
	...
	- @hydra clients create --endpoint http://127.0.0.1:9001 --id cityEx1 --name Example of City1 --secret secret --grant-types authorization_code,refresh_token --response-types code,id_token --scope openid,email,VerifiableIdCredential,VidGoogleCredential,EmailCredential --callbacks http://127.0.0.1:9010/callback
	- sleep 5
	@echo "Running the hydra client..."
	- @hydra token user --skip-tls-verify --endpoint http://127.0.0.1:9001 --port 9010 --auth-url http://127.0.0.1:9000/oauth2/auth --token-url http://127.0.0.1:9000/oauth2/token --client-id cityEx1 --client-secret secret --scope openid,email,VerifiableIdCredential,EmailCredential &
