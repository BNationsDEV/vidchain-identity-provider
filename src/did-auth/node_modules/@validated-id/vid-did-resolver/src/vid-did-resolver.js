...
function wrapDidDocument(did, owner, history) {
  ...
  for (const event of history) {
    const { validTo } = event;
    const key = `${event._eventName}-${event.delegateType || event.name}-${
      event.delegate || event.value
    }`;
      ...
      if (event._eventName === "DIDDelegateChanged") {
        ...
        switch (delegateType) {
          ...
          case "veriKey":
            ...
            break;
          default:
            throw new Error(`method ${delegateType} not supported`);
        }
        ...
        if (match) {
          ...
          switch (section) {
            ...
            case "svc":
              ...
              break;
            default:
              throw new Error(`method ${encoding} not supported`);
          }
        }
      }
    } else {
...
function configureProvider(conf = {}) {
  ...
  return new HttpProvider(conf.rpcUrl || "https://ropsten.infura.io/v3/");
}
...
function getResolver(conf = {}) {
  ...
  async function changeLog(identity, networkId) {
    ...
    if (previousChange) {
      const ownerRecord = await networks[networkId].didReg.identityOwner(
        identity
      );
      owner = ownerRecord["0"];
    }
  ...
  async function resolve(did, parsed) {
    ...
    if (!fullId) throw new Error(`Not a valid vid DID: ${did}`);
    const id = fullId[2];
    const networkId = !fullId[1] ? "mainnet" : fullId[1].slice(0, -1);

    if (!networks[networkId])
      throw new Error(`No conf for networkId: ${networkId}`);
